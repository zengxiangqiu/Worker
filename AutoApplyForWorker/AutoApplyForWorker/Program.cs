...
namespace AutoApplyForWorker
{
    class Program
    {
        private static int noiseThreshold = 1;
        private static int minWidth = 8;
        private static bool isFourChars = false;
        private static int graytype = 2;
        ...
        static void Main(string[] args)
        {
            ...
            for (int i = 0; i < 10; i++)
            {
                ...
                {
                    var config = new ApplyConfig { Index = (int)t, cookieContainer = new CookieContainer(), token = "", username = applicant };
                    while (!login(config)) ;
                    apply(config);
                }, i));
            }
        ...
        static bool login(ApplyConfig config)
        {
            GetToken(config);
            DownloadImageCheck(config);
            var uri = "http://210.76.66.109:7006/gdweb/ajaxlogin.do";
            using (var client = new HttpClient(new HttpClientHandler { CookieContainer = config.cookieContainer }))
            {
                client.DefaultRequestHeaders.Add("Token", config.token);
                client.DefaultRequestHeaders.Add("X-Requested-With", "XMLHttpRequest");

                var imagecheck = GetAuthCode(config);
                var content = new FormUrlEncodedContent(new[]
                {
                        new KeyValuePair<string,string>("CAMY",""),
                        new KeyValuePair<string,string>("des_key","0192217hnisi"),
                        new KeyValuePair<string,string>("IMAGCHECK",imagecheck),
        ...
        static void apply(ApplyConfig config)
        {
            while (true)
            {
                DownloadImageCheck(config);
   ...
   //shareArguments: % 7B % 7D
        ...
        static async void GetToken(ApplyConfig config)
        {
            ...
            using (var client = new HttpClient(new HttpClientHandler { CookieContainer = config.cookieContainer }))
            {
                ...
                config.cookieContainer.Add(new Uri("http://210.76.66.109:7006"), new Cookie("token", val));
                config.token = val;
            }
        }

        static string GetAuthCode(ApplyConfig config)
        {
            Image img2 = Image.FromFile(config.username + config.Index + "\\ImageCheck.jpg");
            string sb = ImageProcess.GetYZMCode(img2, config.username + "\\zimo.txt", noiseThreshold, 20, 30, graytype, isFourChars, minWidth);
            img2.Dispose();
            if (sb.Length > 4)
        ...
        static void DownloadImageCheck(ApplyConfig config)
        {
            Console.WriteLine("Downloading ImageCheck...");
            var uri = "http://210.76.66.109:7006/gdweb/ImageCheck.jpg";
            var httpHandler = new HttpClientHandler { CookieContainer = config.cookieContainer };
            using (var client = new HttpClient(httpHandler))
            {
                if (!Directory.Exists(config.username + config.Index))
                    Directory.CreateDirectory(config.username + config.Index);
                var filename = config.username + config.Index + "\\ImageCheck.jpg";
